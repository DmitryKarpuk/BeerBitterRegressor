...
def predict(dataset_path: Path, submission_path: Path, model_path: Path) -> None:
    # Load data and prepare data
    data = pd.read_csv(dataset_path, index_col=['id'])
    df = clean_data(data)
    df = add_new_features(df)
    X = df.values
    cat_features_idx = [list(df.columns).index(i)
    ...
    click.echo(click.style(f"Submission is saved to {submission_path}",
                                                             fg="green"))